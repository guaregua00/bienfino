
  <div class="content-wrapper">

    <section class="content-header">
      <h1>
        Historico de operaciones de reporte de pago
        <?php
        ?>
        <small></small>
      </h1>
      <ol class="breadcrumb">
        <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
        <li class="active">Dashboard</li>
      </ol>
    </section>
    <br>
    <br>


    <!-- Main content -->
    <section class="content">

      <div class="row">
   
        <div class="col-md-12">

                <table id="listahistoricopago" class="display" cellspacing="0" width="100%">
                    <thead>
                        <tr>
                          <th class="col-md-1">Id Pago Historico</th>
                          <th>Id Publicación</th>                      
                          <th>Id Pago</th>                          
                          <th class="col-md-1">Administrador</th>
                          <th class="col-md-1">Comentario Adm</th>
                          <th>Operación</th>
                          <th>Hora Operación</th>
                        </tr>
                    </thead>

                    <tbody>
                      <?php
                      if (isset($pagohistorico) AND $pagohistorico!="") {
                      foreach($pagohistorico as $value){
                      ?>

                      <tr>
                        <?php
                          echo "<th>".$value->id_pagos_historico."</th>";
                          echo "<th>".$value->id_publicacion."</th>";
                          echo "<th>".$value->id_pago."</th>";                          
                          echo "<th>".$value->usuario."</th>";
                          echo "<th>".$value->comentario_adm."</th>";
                          echo "<th>".$value->operacion."</th>";
                          echo "<th>".$value->fecha_now."</th>";                      
                            ?>
                          </tr>
                          <?php } }?>
                    </tbody>
                </table>

        </div>
      </div>


    </section>

  </div>


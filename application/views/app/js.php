<script>
	...
	function send( $arr ){
		...
		$xhr.onload = async _ => {
			if( $xhr.response ){
				...
				try{
					...
					if( $xhr.response.includes('The action you have requested is not allowed') ){

						var $alerts		= document.querySelector('.alerts-page-container');
						var $alerts_c	= document.querySelector('.alerts-container');
						$alerts_c.innerHTML = '<b>El servidor no reconoce su token de seguridad. Recarge la página para asignarle uno nuevo, si el problema persiste verifique que la URL no sea de una página falsa intentando obtener sus datos.</b>';
					...
					else if( $xhr.response.includes('POST Content-Length of') ){

						var $alerts		= document.querySelector('.alerts-page-container');
						var $alerts_c	= document.querySelector('.alerts-container');
						$alerts_c.innerHTML = '<b>Uno de los archivos enviados ha excedido el límite de envio al servidor. Por favor reduzca el tamaño de sus archivos según lo solicitado e intente nuevamente</b>';
						$alerts.classList.remove('none');
						...
						var $alerts		= document.querySelector('.alerts-page-container');
						var $alerts_c	= document.querySelector('.alerts-container');
						$alerts_c.innerHTML = '<b>Se ha encotrado un error al comunicarse con el servidor. Si el problema persiste notifiquelo al administrador</b>';
						$alerts.classList.remove('none');
		...
		$xhr.onerror = _ => {
			var $alerts		= document.querySelector('.alerts-page-container');
			var $alerts_c	= document.querySelector('.alerts-container');
			$alerts_c.innerHTML = 'Se ha encontrado un error al conectarse con el servidor. Este error puede estar relacionado a su conexión con internet o el servidor no se encuentra disponible en estos momentos.';
			$alerts.classList.remove('none');
